...
func.func @quantize_affine_zp_size_mismatch(%input: tensor<128x64xf32>,
    ...
    %init: tensor<128x64xi8>) -> tensor<128x64xi8> {
  // expected-error @+1 {{expected quantization parameter dimension 0 to have size 64 to match dimension 1 of the quantized value, got 32}}
  %0 = iree_linalg_ext.quantize_affine
      {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                        affine_map<(d0, d1) -> (d0)>,
                        affine_map<(d0, d1) -> (d1)>,
...
func.func @quantize_affine_scale_size_mismatch(%input: tensor<128x64xf32>,
  ...
  // expected-error @+1 {{expected quantization parameter dimension 0 to have size 128 to match dimension 0 of the quantized value, got 64}}
  %0 = iree_linalg_ext.quantize_affine
      {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                        affine_map<(d0, d1) -> (d0)>,
                        affine_map<(d0, d1) -> (d0, d1)>],
