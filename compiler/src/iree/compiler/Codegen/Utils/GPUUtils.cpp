...
inferLdMatrixParamsFromLayout(IREE::VectorExt::NestedLayoutAttr layout,
  ...
  // Need at least rank 2
  int64_t rank = outerTile.size();
  if (rank < 2) {
    LLVM_DEBUG(DBGS() << "ldmatrix: layout rank < 2\n");
    return std::nullopt;
  }

  // Extract last 2 dims (the MMA tile dimensions)
  int64_t outer0 = outerTile[rank - 2];
  int64_t outer1 = outerTile[rank - 1];
  int64_t stride0 = threadStrides[rank - 2];
  int64_t stride1 = threadStrides[rank - 1];

  // numTiles = product of outer tile dimensions
  int numTiles = outer0 * outer1;
  if (numTiles != 1 && numTiles != 2 && numTiles != 4) {
    LLVM_DEBUG(DBGS() << "ldmatrix: invalid numTiles=" << numTiles
                      << ", must be 1, 2, or 4\n");
    return std::nullopt;
  }

  // transpose based on thread stride pattern
  // column-major (stride0 < stride1) needs transpose
  bool transpose = (stride0 < stride1);

  LLVM_DEBUG(DBGS() << "ldmatrix: inferred numTiles=" << numTiles
                    << ", transpose=" << transpose << "\n");
